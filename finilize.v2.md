...
## بازبینی مستقل ۲۰۲۶-۰۹-۰۸

- اجرای مستقل تست‌های غیر Mermaid: **۳۶۳ passed، ۱ skipped، ۵ deselected**؛ skip به نبود LibreOffice مربوط است.
- اجرای مستقیم `B00,C01,R03 × purple_book`: هر ۳ تبدیل و اوراکل‌ها pass شدند، اما پنج هشدار caption بدون تصویر در B00 و سه هشدار متناظر در C01/R03 همچنان بخشی از قرارداد منبع‌اند و باید در manifest به‌صورت صریح ثبت شوند.
- تست Mermaid واقعی `test_f12_real_mermaid_writes_svg_sidecar` در این محیط به‌علت شکست launch مرورگر مدیریت‌شده fail شد؛ این قابلیت بسته نشده و نباید به‌عنوان pass گزارش شود.
- رندر صفحه و مرور Word در محیط حاضر انجام نشده‌اند (`soffice` و Word موجود نیستند)؛ وضعیت درست آن‌ها `pending/blocked` است.
- یک خطای گزارش‌دهی در `scripts/matrix_runner.py` اصلاح شد: شمار `conversions_passed` اکنون از `conversion_success` (وجود DOCX غیرخالی) محاسبه می‌شود و به نتیجهٔ اوراکل‌ها وابسته نیست.
- پس از اصلاح، دو تست رگرسیون گزارش ماتریس pass شدند. هیچ ادعای بسته‌شدن کامل پروژه یا تأیید بصری ثبت نمی‌شود.
