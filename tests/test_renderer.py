...
def test_normal_style_has_cs_font_and_bidi(renderer):
    ...
    # RTL paragraph_align=start uses logical "start" — both Word and LibreOffice render right-aligned
    assert 'w:jc w:val="start"' in xml
...
def test_render_paragraph_rtl(renderer):
    ...
    xml = p._p.xml
    assert "<w:bidi" in xml
    # paragraph-level jc=start aligns right in RTL (logical value, not flipped)
    assert 'w:jc w:val="start"' in xml
    assert 'w:cs="Vazirmatn"' in xml
